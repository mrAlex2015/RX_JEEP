...
// Battery type
boolean liPo = false;
float cutoffVoltage = 3.0; // 5V receiver supply voltage surveillance from BEC only!
...
byte lim1L = 45, lim1R = 145; // R67  L137
byte lim2L = 45, lim2R = 120; // Gearbox shifter limits (1. and 2. gear)
byte lim3L = 55, lim3R = 125; // +/-25° is still full throttle with the JMT-10A ESC! (Forward, Reverse)
byte lim3Llow = 65, lim3Rlow = 105; // same setting (full throttle), because of shifting gearbox!
byte lim4L = 35, lim4R = 145;
#define TWO_SPEED_GEARBOX // Vehicle has a mechanical 2 speed shifting gearbox, switched by servo CH2. Not usable in combination with the "tailLights" option

// Motor configuration
int maxPWMfull = 255;
int maxPWMlimited = 150;
int minPWM = 0;
/* Время разгона со старта - влияет так-же на выполнение реверса, хода назад
 3 - хорошо воспринимает старт переход в реверс тяжело управлять при старте уходит в занос
 12 - устойчивее при старте, но также уходит в занос
 25 - устойчивее при старте, но также уходит в занос если передержать но послушней при смене поворота руль (отклик по влево-право лучше)
 */
byte maxAccelerationFull = 4;
